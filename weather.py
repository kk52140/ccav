...
def get_weather_warning(weather_type, high_temp, low_temp):
    ...
    elif high <= 5 or low <= 0:
        reminders.append("气温偏低，外出请注意添衣保暖。")
    elif high <= 10 or low <= 5:
...
def build_weather_notice(weather_type, high_temp, low_temp, quality):
    """生成更适合物业场景的天气提示"""
    high = extract_temp(high_temp, 20)
    low = extract_temp(low_temp, 10)
    ...
    if "晴" in weather_type:
        ...
        notices.append("天气以阴天为主，外出请留意体感变化。")

    if "雨" in weather_type:
        notices.append("外出建议随身携带雨具。")
    if "雪" in weather_type:
        notices.append("请留意路面湿滑，出行注意安全。")
    if "风" in weather_type:
        notices.append("请留意高空坠物，并妥善安置阳台物品。")
